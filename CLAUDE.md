...
- **Home page** ([src/app/page.tsx](src/app/page.tsx)) is a stack of `<section id="...">` blocks (`hero`, `about`, `project`, `contact`). The navbar ([src/components/Navbarr.tsx](src/components/Navbarr.tsx)) scrolls to these IDs with `react-scroll`, driven by `navbarItem` in [src/constants/index.ts](src/constants/index.ts) — keep section IDs and `navbarItem` in sync. `Navbar.tsx` and `Projects.tsx` are older unused variants; `Navbarr.tsx` and `Project.tsx` are the live ones.
...
- **UI components**: HeroUI v2 (navbar, popover) and shadcn/ui ([src/components/ui/](src/components/ui/), [components.json](components.json)). The toast system (`useToast` + `<Toaster/>` in the layout) is shadcn's.
- **Contact flow**: [src/components/Contact.tsx](src/components/Contact.tsx) POSTs to `/api/contact` ([src/app/api/contact/route.ts](src/app/api/contact/route.ts)), which saves to MongoDB via the Mongoose `Contact` model ([src/model/Contact.ts](src/model/Contact.ts)). `GET /api/contact` returns all messages and is unauthenticated. The model uses the `mongoose.models.Contact ||` pattern to survive hot reload; `connect()` is called per request.
